...
class Helper_Plupload {
	...
	public function Upload($destination_dir, $force = false, $destination_name = false) {
		...
		// Grab request data
		$t = Request_Vars::Instance()->getVars('request');
		
		#file_put_contents(APPLICATION_PATH.'/upload.log', print_r($_REQUEST,true)."\n", FILE_APPEND);
		
		// Hijack session :-( the saddest part of this script
		session_id($t['sid']);
		
		// Check is user loged (session authorization)
		if (!Auth::Instance()->isLoged() && !$force) {
