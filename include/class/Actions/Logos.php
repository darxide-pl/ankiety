...
class Logos_Actions extends Action_Script {
	...
	public function _add() {
		...
		if ($this->pass()) {
			...
			# get new position
			$position = (int) $this->db->FetchRecord("SELECT MAX(`pos`) FROM `logo`;",DB_FETCH_ARRAY_FIELD);
			...
			$this->request->execute($this->link->Build(array('logos','edit','oid'=>$o->get('id'))), true);
		}
	}
	...
	public function _update() {
		...
		if ($this->pass()) {
			...
			if ($t['parent_id'] != $o->get('parent_id')) {
				...
				# get new position
				$position = (int) $this->db->FetchRecord("SELECT MAX(`pos`) FROM `logo` WHERE `parent_id` = '".$t['parent_id']."';", DB_FETCH_ARRAY_FIELD);
		...
		$this->request->execute($this->link->Build(array('logos','edit','oid'=>$t['id'])),true);
	}
	...
	public function _delete() {
		...
		if ($this->pass()) {
			...
			try {
				...
				$object = new Model_Logo($t['id']);
				$object->delete();

				$this->msg('Baner <b>'.$object->get('name').'</b> został usunięty.');
				
				// remove old image
				if (file_exists(DIR_UPLOAD . 'logo/'.$object->get('image_filename'))) {
					unlink(DIR_UPLOAD . 'logo/'.$object->get('image_filename'));
					unlink(DIR_UPLOAD . 'logo/'.'thumbs/'.$object->get('image_filename'));
					unlink(DIR_UPLOAD . 'logo/'.'grey/'.$object->get('image_filename'));
				}
	...
	public function _ajax_save_positions() {
		...
		Helper_Logos::Update_Order($order);
	}
	...
	public function _upload_image() {
		...
		if (!$chunks || $chunk == $chunks - 1) {
			...
			$destination_dir = DIR_UPLOAD . 'logo/';
			...
			$img->width(Config::get('logo.width_thumb'))
				->height(Config::get('logo.height_thumb'))
				->scaleToBox(true)
				->save($destination_dir.'thumbs/'.$filename);
			
			$img = new ImageHandler($destination_dir . $filename);
			$img->width(Config::get('logo.width'))
				->height(Config::get('logo.height'))
				->scaleToBox(false)
				->save($destination_dir.$filename);
			
			$object = new Model_Logo($oid);
			...
			die(DIR_UPLOAD_ABSOLUTE.'/logo/'.$filename);
		}
