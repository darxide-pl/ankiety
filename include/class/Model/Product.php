...
class Model_Product extends Generic_Object {
	...
	public static function Build_Url($o) {
		...
		if (is_array($o)) {
			return Link::Instance()->Build('sklep/'.self::Build_Path($o['category_id']).$o['alias'].'.html',array('domain'=>BASE_DOMAIN.BASE_DIR.LT));
		} else if (is_object($o)) {
			return Link::Instance()->Build('sklep/'.self::Build_Path($o->get('category_id')).$o->get('alias').'.html',array('domain'=>BASE_DOMAIN.BASE_DIR.LT));
		}
	}
