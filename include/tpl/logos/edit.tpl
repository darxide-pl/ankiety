	...
	<div class="panel panel-primary">
		...
		<div class="panel-body">
			...
			<form action="" method="post" class="form-horizontal" id="submit-form">
				...
				<input type="hidden" name="page_action" value="<?php echo f_eas('Logos|update') ?>" />
				...
				<div class="form-group">
					<div class="col-sm-9 col-sm-offset-3">
						<div class="img-thumbnail" id="news-image">
							...
							<img src="<?php echo DIR_UPLOAD_ABSOLUTE ?>/logo/<?php echo $object->get('image_filename') ?>" class="img-responsive" alt="" title="" />
							<?php endif ?>
						</div>
					</div>
				</div>
				...
				<div class="form-group">
					<label class="control-label col-sm-3">Grupa</label>
					<div class="col-sm-9"><select name="parent_id" class="form-control">
						<option value="0">Brak</option>
						<?php echo Html::SelectOptions($groups,(int)$object->get('parent_id')) ?></select></div>
				</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-9"><textarea name="description" id="page-text" class="form-control" rows="4"><?php echo ($object->get('description')) ?></textarea></div>
				</div>
...
window.addEventListener("unload", function (e) {
	...
	if (close_by_submit === false && has_changes === true) {
		...
		$.ajax({
			type: 'POST',
			async: false,
			url: '<?php echo $this->link->Build(array($this->name,'index','page_action'=>f_eas('Logos|delete'),'id'=>$object->get('id'))) ?>',
			data: ''
		});
		<?php endif; ?>
	}
...
function lock_refresh() {
	$.get('<?php echo $this->link->Build(array($this->name,$this->view,'page_action'=>f_eas('System_Locks|lock'),'type'=>'logos','object_id'=>$object->get('id'))) ?>',{},function(data){
...
<script type="text/javascript">
	var uploader = new plupload.Uploader({
		...
		multipart: true,
		url : '<?php echo $this->link->Build(array($this->name,$this->view,'page_action'=>f_eas('Logos|upload_image'),'sid'=>session_id(),'id'=>$object->GetID())) ?>',
