		...
		<div class="panel-heading">
			...
			<div class="btn-group pull-right">
				<a href="<?php echo $this->link->Build(array($this->name,'add')) ?>"  class="btn btn-sm btn-success">
					<i class="glyphicon glyphicon-plus"></i> Nowe wydarzenie</a>
			</div>
		</div>
		...
		<div class="table-responsive">
			...
			<thead>
				<tr>
					...
					<th><?php echo $this->t('Gdzie','Gdzie') ?></th>
					<th><?php echo $this->t('Kiedy','Kiedy') ?></th>
					<th><?php echo $this->t('Galeria','Galeria') ?></th>
			...
			<tr class="row<?php echo ($i = $i == 1 ? 2 : 1);?> vmiddle">
				...
				<td>
					...
					<a href="<?php echo $this->link->Build(array($this->name,'edit','oid'=>$o->GetID())) ?>"><?php echo $o->get('name') ?></a>
				</td>
				<td><?php echo $o->get('where') ?></td>
				
				<td><?php echo $o->get('from') > 0 ? $o->get('from') : '' ?><br />
					<?php echo $o->get('to') > 0 && $o->get('to') != $o->get('from') ? $o->get('to') : '' ?></td>
				<td></td>
				<!--td><?php echo $o->get('views') ?></td-->
				<td><small><?php echo f_format_date('d.m.Y H:i',$o->get('modify_date')) ?></small>
				...
				<td><a href="<?php echo $this->link->Build(array($this->name,$this->view,'page_action'=>f_eas('Events|delete'),'id'=>$o->get('id'))) ?>" class=" btn btn-danger confirm" title="Czy usunąć wydarzenie?"><i class="glyphicon glyphicon-remove"></a></td>
			</tr>
...
function locks_refresh() {
	...
	$.get('<?php echo $this->link->Build(array($this->name,$this->view,'page_action'=>f_eas('System_Locks|check_locks'),'type'=>'events')) ?>',objects,function(data){
...
$(document).ready(function(){
	...
	$('.btn-toggle-publish').click(function(e){
		e.preventDefault();
		var $this = $(this), label = '';
		$.post('<?php echo $this->link->Build(array($this->name,$this->view,'page_action'=>f_eas('Events|switch_publish'))) ?>',{id:$this.data('id')},function(){
