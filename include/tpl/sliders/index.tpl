<div class="container">
	<div class="panel panel-default">
		<div class="panel-heading">
			...
			Slider&nbsp;<span class="badge"><?php echo $this->list->getAmount() ?></span>
			
			<div class="btn-group pull-right ">
				<a class="btn btn-default" href="<?php echo $this->link->Build(array($this->name,'add')) ?>"><i class="glyphicon glyphicon-plus"></i><span class="hidden-xs"> &nbsp; Dodaj slajd</span></a>
			...
			<div class="table-responsive">
				...
				<tr class="row<?php echo ($i = $i == 1 ? 2 : 1);?> vmiddle">
					...
					<td><?php if ($o->get('image_filename')): ?>
						<img src="<?php echo DIR_UPLOAD_ABSOLUTE ?>/slider/thumbs/<?php echo $o->get('image_filename') ?>" class="img-polaroid" alt="" title="" />
						<?php endif ?></td>
					...
					<td><a href="<?php echo $this->link->Build(array($this->name,$this->view,'page_action'=>f_eas('Sliders|delete'),'id'=>$o->get('id'),'parent_id'=>$o->get('parent_id'))) ?>" class="confirm btn btn-danger" title="Czy usunąć baner?"><i class="glyphicon glyphicon-remove"></a></td>
				</tr>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<div class="form-group">
						<label class="col-md-4 control-label">Pokaż slider</label>
						<div class="col-md-8">
							<label class='radio-inline'>
								<input type="radio" name="cfg[slider][show]" value="0" checked /> Nie
							</label>
							<label class='radio-inline'>
								<input type="radio" name="cfg[slider][show]" value="1" <?php echo Config::get('slider.show') ? 'checked' : '' ?> /> Tak
							</label>
						</div>
					</div>
					
					<div class="form-group">
						<label class="col-md-4 control-label">Wymiary slidera</label>
						<div class="col-md-4">
							<div class="input-group">
								<input type="text" name="cfg[slider][width]" placeholder="szerokość" value="<?php echo Config::get('slider.width') ?>" class="form-control" />
								<span class="input-group-addon">px</span>
							</div>
						</div>
						<div class="col-md-4">
							<div class="input-group">
								<input type="text" name="cfg[slider][height]" placeholder="wysokość" value="<?php echo Config::get('slider.height') ?>" class="form-control" />
								<span class="input-group-addon">px</span>
							</div>
						</div>
					</div>
					...
					<div class="form-group">
						...
						<div class="col-md-4">
							<div class="input-group">
								<input type="text" name="cfg[slider][width_thumb]" placeholder="szerokość" value="<?php echo Config::get('slider.width_thumb') ?>" class="form-control" />
								<span class="input-group-addon">px</span>
							</div>
						</div>
						<div class="col-md-4">
							<div class="input-group">
								<input type="text" name="cfg[slider][height_thumb]" placeholder="wysokość" value="<?php echo Config::get('slider.height_thumb') ?>" class="form-control" />
								<span class="input-group-addon">px</span>
							</div>
						</div>
					</div>
...
function locks_refresh() {
	...
	$.get('<?php echo $this->link->Build(array($this->name,$this->view,'page_action'=>f_eas('System_Locks|check_locks'),'type'=>'slider')) ?>',objects,function(data){
...
$(document).ready(function(){
	...
	$('.btn-toggle-publish').click(function(e){
		e.preventDefault();
		var $this = $(this), label = '';
		$.post('<?php echo $this->link->Build(array($this->name,$this->view,'page_action'=>f_eas('Sliders|switch_publish'))) ?>',{id:$this.data('id')},function(){
	...
	$("#sortable-table").sortable({
		...
		update: function(){
			...
			$.post('<?php echo $this->link->Build(array($this->name,$this->view,'page_nr'=>$this->list->GetData('page_nr'),'page_action'=>f_eas('Sliders|ajax_save_positions'))) ?>',vars,function(data){
				// result
			},'json');
		}
	});//.disableSelection();
	
	$('input[name="cfg[slider][width_thumb]"]').keyup(function(e){
		var w = $('input[name="cfg[slider][width]"]').val(),
			h = $('input[name="cfg[slider][height]"]').val(),
			wt = $(this).val();
		$('input[name="cfg[slider][height_thumb]"]').val(Math.round((wt*h)/w));
	});
});
