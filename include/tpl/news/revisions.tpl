...
<div id="content" class="content-noright">
	...
	<ul class="breadcrumb">
		...
		<li><a href="<?php echo $this->link->Build(array($this->name)) ?>">Aktualności</a> <span class="divider">/</span></li>
		...
		<?php echo text_diff($revision->get('text'), $object->get('text'),
				array(
					'title_left'=>'Wersja z dnia: '.$revision->get('add_date').'
						<a href="'.$this->link->Build(array($this->name,'edit','oid'=>$revision->get('news_id'),'page_action'=>f_eas('News|revive_copy'),'rid'=>$revision->get('id'))).'" class="btn btn-success confirm pull-right" title="Czy na pewno chcesz przywrócić tą wersję strony?">Przywróć</a>',
	...
	<form action="" class="" style="clear: both;" method="post" id="lf">
		...
		<thead>
			<tr>
				<th width="10">&nbsp;</th>
				<th><?php echo $this->t('News','Aktualność') ?></th>
		...
		<tr class="row<?php echo ($i = $i == 1 ? 2 : 1);?> vmiddle">
			<td></td>
			<td><a href="<?php echo $this->link->Build(array($this->name,$this->view,'oid'=>$o->get('news_id'),'rid'=>$o->get('id'))) ?>"><?php echo $o->get('title') ?></a></td>
			...
			<td><div class="btn-group">
					<a href="<?php echo $this->link->Build(array($this->name,'edit','oid'=>$o->get('news_id'),'page_action'=>f_eas('News|revive_copy'),'rid'=>$o->get('id'))) ?>" class="btn btn-small btn-success confirm" title="Czy na pewno chcesz przywrócić tą wersję strony?">Przywróć</a>
					<a href="<?php echo $this->link->Build(array($this->name,$this->view,'oid'=>$o->get('news_id'),'rid'=>$o->get('id'),'compare'=>1)) ?>" class="btn btn-small">Porównaj z obecną wersją</a>
				</disv></td>
		</tr>
